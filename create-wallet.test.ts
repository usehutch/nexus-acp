...
describe("createDevnetWallet", () => {
    ...
    test("should create a new wallet with valid keypair", async () => {
        const wallet = await createDevnetWallet({ testMode: true });
    ...
    test("should generate unique wallets on multiple calls", async () => {
        const wallet1 = await createDevnetWallet({ testMode: true });
        ...
        const wallet2 = await createDevnetWallet({ testMode: true });
